...
public function index()
{
   ...
   if($jwtData && $jwtData->user_group == "admin"):
       ...
       $this->paginate =  [
         'limit' => 5 ,
         'order'=> ['Contacts.id'=>'DESC' ] //id
        ] ;
      $index = $this->paginate($this->Contacts);
      $pagination = $this->request->getAttribute('paging')['Contacts'];

       if($index){
         $msg = "قائمة  تواصل العملاء"  ; $success = true ;   $data = $index ; 
      } else{
         //not found
