...
class CommentsTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator ->notEmptyString('comment',"التعليق مطلوب");

        $validator ->notEmptyString('article_id',"المقال مطلوب");
        return $validator;
    }


    ////////////
        function addNewComment($arr){
           $new = $this->newEmptyEntity();
           $new = $this->patchEntity($new,$arr);
           if($this->save($new)){
            $msg = "تم اضافة التعليق بنجاح وسيتم مراجعته من خلال الادمن";$success = true ; $data = $new ; 
            }else{
